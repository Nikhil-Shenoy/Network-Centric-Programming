...
int main() {
  ...
  while(1) { // endless loop to handle multiple connections
    ...
    // *** Changed sprintf to safer snprintf. Made sure the right number of characters are being read into command.
    if((strlen(packet) + 2) < (MAX_FILENAME_SIZE + 8)) {
   	 snprintf(command,strlen(packet), "[ -e %s ]", pkt_filename);
    else 
	fprintf(stderr,"Error in extracting command\n");
	...
	// *** Changed strcpy to safer strncpy. Made sure the appropriate number of bytes are being read in.
	if(strlen(PKT_MODE(packet)) < MAX_MODE_LENGTH)
		strncpy(modeString, PKT_MODE(packet),strlen(PKT_MODE(packet)));
	else
		fprintf(stderr,"Error in extracting the mode string\n");

	// *** Changed strcasecmp to safer strncasecmp
    if(strncasecmp(modeString, "octet",strlen("octet")) == 0)
      mode = MODE_OCTET;
    else if(strncasecmp(modeString, "netascii",strlen("netascii")) == 0) {
      mode = MODE_NETASCII;

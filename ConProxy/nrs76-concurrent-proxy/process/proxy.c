...
int logfd; // file descriptor for the logfile. Child processes get "identical copies of parent's open file descriptors
...
void process(int connfd, struct flock fileLock, struct sockaddr_in clientaddr);
...
listenfd = Open_listenfd(port);
	...
	mode_t mode = S_IRWXU | S_IRWXG | S_IRWXO;
  	logfd = open(PROXY_LOG,O_WRONLY | O_APPEND | O_CREAT,mode);
	...
	while (1) { 
		...
		if((childPid = fork()) == 0) {
			...
			*/
			...
			fcntl(logfd,F_SETLKW,&fileLock);

			format_log_entry(log_entry, &clientaddr, request_uri, response_len);  
			char entry[strlen(log_entry) + 8]; // at most six more digits for the size, 1 for the space
			char response_length[6];
			strcpy(entry,log_entry);
			strcat(entry," ");
			sprintf(response_length,"%d\n",response_len);
			strcat(entry,response_length);
			write(logfd,entry,strlen(entry));
			close(logfd);
   
			// Unlock the file
			fileLock.l_type = F_UNLCK;
			fcntl(logfd,F_SETLK,&fileLock);

...
pthread_mutex_t functionLock;
pthread_mutex_t fileLock;

/*
 * Functions not provided to the students
 */
int open_clientfd(char *hostname, int port);  // unsafe
ssize_t Rio_readn_w(int fd, void *ptr, size_t nbytes); // unsafe
ssize_t Rio_readlineb_w(rio_t *rp, void *usrbuf, size_t maxlen); // unsafe 
void Rio_writen_w(int fd, void *usrbuf, size_t n); // unsafe

/*
 * Function prototypes
 */
int parse_uri(char *uri, char *target_addr, char *path, int  *port); // unsafe
void format_log_entry(char *logstring, struct sockaddr_in *sockaddr, char *uri, int size); // unsafe

/*
 * Struct that contains argument for each thread
*/
struct argStruct {
...
/*
 * Worker thread function. Contains all the logic for processing
*/
void *process(void *args) {

	struct argStruct *threadArgs;
	threadArgs = (struct argStruct *)args;

	struct sockaddr_in *clientaddrPtr;
	int connfd;

	clientaddrPtr = threadArgs->ClientAddrPtr;
	connfd = threadArgs->Connfd;
		...
		pthread_mutex_lock(&functionLock);
                Rio_readinitb(&rio, connfd); // file descriptor is shared resource
		pthread_mutex_unlock(&functionLock);
		...
		/*
                * Log the request to disk. Perform the lock first
                */
		pthread_mutex_lock(&fileLock);

                format_log_entry(log_entry, clientaddrPtr, request_uri, response_len);
...
int main(int argc, char **argv)
{
	...
	pthread_mutex_init(&functionLock,NULL);
	...
	*/
	...
	while (1) { 
		...
		struct argStruct threadArgs;
		struct argStruct *threadArgsPtr;
		threadArgs.ClientAddrPtr = &clientaddr;
		threadArgs.Connfd = connfd;
		threadArgsPtr = &threadArgs;

		pthread_t tid;
		int rValue;
		rValue = pthread_create(&tid,NULL,process,(void *)threadArgsPtr);	
		if(rValue != 0) 
			continue;
